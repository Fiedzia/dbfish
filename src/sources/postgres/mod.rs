...
pub fn get_postgres_url(postgres_options: &dyn GetPostgresConnectionParams) -> String {
    format!(
        ...
        user = match &postgres_options.get_username() {
            None => "",
            Some(v) => v,
        },
...
impl PostgresSource {
    pub fn init(postgres_options: &PostgresSourceOptions) -> PostgresSource {
        PostgresSource {
            options: postgres_options.to_owned(),
        }
    }
}

impl<'source: 'conn, 'conn> PostgresSourceConnection<'source> {
    pub fn _batch_iterator(
        source: &'source PostgresSource,
        connection: &'source mut Client,
        batch_size: u64,
...
impl<'source: 'conn, 'conn> DataSource<'source, 'conn, PostgresSourceConnection<'source>>
    for PostgresSource
{
...
pub fn postgres_to_row(
    column_info: &[(String, postgres::types::Type)],
    postgres_row: &postgres::row::Row,
) -> Row {
