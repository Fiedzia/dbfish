...
pub fn value_to_ods_value(value: &Value, truncate: Option<u64>) -> spreadsheet_ods::Value {
    match value {
        ...
        Value::String(value) => spreadsheet_ods::Value::Text(truncate_text_with_note(value.to_string(), truncate)),
        ...
        Value::Timestamp(value) => spreadsheet_ods::Value::DateTime(chrono::NaiveDateTime::from_timestamp(*value as i64, 0)), 
        Value::Date(value) => spreadsheet_ods::Value::DateTime(
            chrono::NaiveDateTime::new(*value, chrono::NaiveTime::from_hms(0, 0, 0))
        ),
        Value::Time(value) => spreadsheet_ods::Value::Text(
            format!("{:?}", value)
        ),
...
impl DataDestination for SpreadSheetODSDestination
{
    ...
    fn add_rows(&mut self, rows: &[Row]) {
        ...
        for row in rows {
            for (idx, val) in row.iter().enumerate() {
                sheet.set_value(self.sheet_row_count as u32, idx as u32, value_to_ods_value(val, self.truncate));
            }
            self.sheet_row_count += 1;
        }
    }
    ...
    fn close(&mut self) {
        spreadsheet_ods::write_ods(&mut self.workbook, self.filename.clone()).expect(&format!("write_ods: {}", self.filename))
    }
}
