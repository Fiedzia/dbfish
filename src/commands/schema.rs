            ...
            Some(root_node_id) => {
                ...
                for node_id in self.0.traverse_post_order_ids(root_node_id).unwrap() {
                    if self
                        ...
                        .unwrap()
                        .data()
                        .matches(query, is_regex)
                    {
                        let mut ancestor_ids: Vec<NodeId> =
                            self.0.ancestor_ids(&node_id).unwrap().cloned().collect();
                        ...
                        for node_id in ancestor_ids {
                            ...
                            let new_node_id = new_dbitems
                                .0
                                .insert(
                                    Node::new(node.data().clone()),
                                    match node.parent() {
                                        None => InsertBehavior::AsRoot,
                                        Some(parent_id) => InsertBehavior::UnderNode(
                                            node_map.get(parent_id).unwrap(),
                                        ),
                                    },
                                )
                                .unwrap();
...
pub fn schema(
    ...
    match &src {
        ...
        DataSourceCommand::Mysql(mysql_config_options) => {
            ...
            for row in results {
                let (schema_name, table_name, column_name, column_type, is_nullable): (
                    String,
                    String,
                    String,
                    String,
